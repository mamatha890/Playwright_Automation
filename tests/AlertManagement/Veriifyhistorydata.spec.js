...
testdata.forEach((code) => {
    //const testType = code.TestType.trim().toLowerCase();

  if (code.TestType === "verify alerts history") {
    ...
    await AlertManagement.headerverification();
    await AlertManagement.verifinghistorydata(data);
    code.Status="Pass"
      }
      catch(err){
